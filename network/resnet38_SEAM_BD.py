...
import network.auxiliary as auxiliary
...
class Net(network.resnet38d.Net):
    def __init__(self):
        ...
        self.fuse_canny_edge = nn.Conv2d(42, 21, kernel_size=1, padding=0, bias=False)
        self.edge_layer = auxiliary.Semantic_Edge_Detection_Module()

    def forward(self, x):
        N, C, H, W = x.size()
        d = super().forward_as_dict(x)
        cam = self.fc8(self.dropout7(d['conv7']))
        n,c,h,w = cam.size()
        with torch.no_grad():
        ...
        semantic_edge = self.edge_layer(d['conv1'], d['conv3'],
                                        d['conv5'], d['conv6'])

        # The following part is to incorporate bottom-up edge information

        im_arr = x.cpu().numpy().transpose((0, 2, 3, 1)).astype(np.uint8)
        canny = np.zeros((N, 1, H, W))
        for i in range(N):
            canny[i] = cv2.Canny(im_arr[i], 10, 100)
        canny = torch.from_numpy(canny).cuda().float().expand(N, 21, H, W)
        # canny = torch.from_numpy(canny).cuda().float()

        # semantic_edge = self.edge_layer(d['conv1'], d['conv3'], d['conv5'], d['conv6'], canny)
        # semantic_edge = self.edge_layer(d['conv1'], d['conv3'], d['conv5'], d['conv6'], canny)

        semantic_edge = self.fuse_canny_edge(torch.cat([semantic_edge, canny], dim=1))

        return cam, cam_rv, semantic_edge
