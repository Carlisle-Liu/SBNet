...
class Net(nn.Module):
    ...
    def forward_as_dict(self, x):
        ...
        x = self.conv1a(x)
        # 64*448*448 / 64*134*134

        x, conv1 = self.b2(x, get_x_bn_relu=True)
        # 128*224*224 / 128*67*67
        x = self.b2_1(x)
        # 128*224*224 / 128*67*67
        x = self.b2_2(x)
        # 128*224*224 / 128*67*67
        ...
        # 2048*56*56 / 2048 * 17 * 17

        x, conv6 = self.b7(x, get_x_bn_relu=True)
        # 4096*56*56 / 4096 * 17 * 17
        conv7 = F.relu(self.bn7(x))

        return dict({'conv1': conv1, 'conv2': conv2, 'conv3': conv3, 'conv4': conv4, 'conv5': conv5, 'conv6': conv6, 'conv7': conv7})
