...
class BatchThreader:
    ...
    def pop_results(self):
        ...
        # print('stage 1 reached: n_inwork')

        n_fetch = min(n_inwork, self.batch_size)
        # print('stage 2 reached: n_fetch： ', n_fetch)
        # print(self.async_result)
        rtn = [self.async_result.pop(0).get() for _ in range(n_fetch)]

        # print('stage 3 reached: rtn')
        to_fill = self.__get_n_pending_works()

        # print('stage 4 reached: to_fill')
        if to_fill == 0:
            # print('stage 5.1 reached: pool_clone')
            self.pool.close()
        else:
            # print('stage 5.2 reached: start_works')
            self.__start_works(to_fill)

        # print('stage 6 reached: before return')
        return rtn
