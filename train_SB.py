...
import voc12.data_BD
from tool import pyutils, imutils, torchutils, visualization, myutils_20
import argparse
import importlib
...
def gradient_map(cam):
    n, c, h, w = cam.shape
    cam = cam.view(1, n*c, h, w)
    laplace = [[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]]
    lap_kernel = torch.tensor(laplace, dtype=torch.float32).unsqueeze(0).expand(n*c, 1, 3, 3)
    lap_kernel = lap_kernel.cuda()
    g_map = F.conv2d(cam, lap_kernel, stride=1, padding=1, groups=c*n)
    g_map = g_map.view(n, c, h, w)
    return g_map
...
if __name__ == '__main__':
    ...
    parser.add_argument("--network", default="network.resnet38_SEAM_BD", type=str)
    ...
    TF_1 = transforms.Compose([myutils_20.RandomResizeLong(448, 768),
                               myutils_20.RandomHorizontalFlip()])

    TF_2 = transforms.Compose([transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1),
                               np.asarray,
                               model.normalize])


    TF_3 = transforms.Compose([myutils_20.RandomCrop(args.crop_size)])

    TF_4 = transforms.Compose([imutils.HWC_to_CHW])

    TF_5 = transforms.Compose([torch.from_numpy])

    train_dataset = voc12.data_BD.VOC12ClsDataset(args.train_list, voc12_root=args.voc12_root,
                                               transform=[TF_1, TF_2, TF_3, TF_4, TF_5])
    ...
    avg_meter = pyutils.AverageMeter('loss', 'loss_cls', 'loss_er', 'loss_ecr', 'loss_ct', 'loss_smooth')
    ...
    for ep in range(args.max_epoches):
        ...
        for iter, pack in enumerate(train_data_loader):
            ...
            # gray1 = pack[4]
            # gray1 = gray1.cuda()
            # gray2 = F.interpolate(gray1, scale_factor=scale_factor, mode='bilinear', align_corners=True)


            cam1, cam_rv1, edge1 = model(img1)

            # gradient_cam_1 = gradient_map(cam1)
            # gradient_cam_1 = torch.mul(visualization.max_norm(gradient_cam_1), label)
            # gradient_cam_1 = torch.max(gradient_cam_1[:, 1:, :, :], dim=1, keepdim=True)[0]

            loss_ct_1 = BCELoss(torch.sigmoid(edge1), ct * label)

            smoothness_loss_1 = smooth_loss(torch.sigmoid(cam1), ct, label)
            ...
            cam2, cam_rv2, edge2 = model(img2)


            # gradient_cam_2 = gradient_map(cam2)
            # gradient_cam_2 = torch.mul(visualization.max_norm(gradient_cam_2), label)
            # gradient_cam_2 = torch.max(gradient_cam_2[:, 1:, :, :], dim=1, keepdim=True)[0]
            ct_2 = F.interpolate(ct, scale_factor=scale_factor, mode='bilinear', align_corners=True)

            smoothness_loss_2 = smooth_loss(torch.sigmoid(cam2), ct_2, label)

            smoothness_loss = 1 * 0.5 * (smoothness_loss_1 + smoothness_loss_2)

            loss_ct_2 = BCELoss(torch.sigmoid(edge2), ct_2 * label)
            ...
            loss_ct = 0.01 * 0.5 * (loss_ct_1 + loss_ct_2)
            ...
            loss = loss_cls + loss_er + loss_ecr + loss_ct + smoothness_loss
            # loss = loss_cls + loss_er + loss_ecr + smoothness_loss
            ...
            avg_meter.add({'loss': loss.item(), 'loss_cls': loss_cls.item(), 'loss_er': loss_er.item(),
                           ...
                           'loss_smooth': smoothness_loss.item()})

            if (optimizer.global_step - 1) % 50 == 0:
                timer.update_progress(optimizer.global_step / max_step)

                print('Iter:%5d/%5d' % (optimizer.global_step - 1, max_step),
                      'loss:%.4f %.4f %.4f %.4f %.4f, %.4f' % avg_meter.get('loss', 'loss_cls', 'loss_er', 'loss_ecr',
                                                                    'loss_ct', 'loss_smooth'),
        ...
        torch.save(model.module.state_dict(), args.session_name + '_' + str(ep) + '.pth')
