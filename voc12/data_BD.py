...
def get_ct_path(img_name, voc12_root):
    return os.path.join(voc12_root, CT_FOLDER_NAME, img_name + '.npy')
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ct = np.load(get_ct_path(name, self.voc12_root))

        if self.transform:
            img, ct = self.transform[0]([img, ct])
            img = self.transform[1](img)
            img, ct = self.transform[2]([img, ct])
            img = self.transform[3](img)
            img = self.transform[4](img)
            ct = self.transform[4](ct)
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        name, img, ct = super().__getitem__(idx)

        label = torch.from_numpy(self.label_list[idx])

        return name, img, label, ct
